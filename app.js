document.addEventListener('DOMContentLoaded', () => {
  // PDF.js 글로벌 설정 (워커 동적 주입 및 폴백 가이드)
  if (window.pdfjsLib) {
  ...
  // 안정화된 PDF 텍스트 추출 함수 (CORS 및 Worker 에러 자동 방어)
  ...
  async function handlePdfUpload(fileInputId, statusId, targetHiddenId) {
    ...
    try {
      ...
      if (!text) {
        statusEl.textContent = '⚠️ 텍스트를 추출할 수 없는 스캔형(이미지) PDF입니다.';
      ...
      statusEl.textContent = '❌ 파일 처리 실패 (인터넷 연결 상태를 확인하거나 텍스트를 직접 복사해 붙여넣어 주세요)';
      statusEl.style.color = '#E11D48';
    }
  }
